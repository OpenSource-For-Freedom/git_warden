...
_VECTOR_BY_RULE = {
    ...
    "npm-preinstall": ("npm install hook (preinstall)", "Runs a fetch/decode-and-execute on npm install."),
    "npm-install": ("npm install hook", "Runs a fetch/decode-and-execute on npm install."),
    "npm-postinstall": ("npm install hook (postinstall)", "Runs a fetch/decode-and-execute on npm install."),
    "npm-prepare": ("npm install hook (prepare)", "Runs a fetch/decode-and-execute on npm install."),
    "npm-preuninstall": ("npm install hook (preuninstall)", "Runs a fetch/decode-and-execute on npm uninstall."),
    "py-setup-exec": ("setup.py fetch-and-run", "setup.py downloads-and-executes a payload at build time."),
    "env-dump": ("Environment / credential exfiltration",
                 "Serializes the whole process environment (secrets, tokens) to send it out."),
    "secret-exfil": ("Secret-file exfiltration", "curl/wget uploads a private key or credentials file."),
    ...
    "bash-i-socket": ("Reverse shell (bash /dev/tcp)", "Interactive bash redirected over a raw TCP socket."),
    "osm-listed": ("Known-malicious dependency", "Declares a version-pinned known-malicious package."),
}

...
def build_run_argv(full_name: str, image: str = SANDBOX_IMAGE) -> list[str]:
...
def scan_candidate_sandboxed(full_name: str, *, image: str = SANDBOX_IMAGE,
                             runner=subprocess.run) -> Tier2Result | None:
    ...
    """
    argv = build_run_argv(full_name, image)
    try:
        proc = runner(argv, capture_output=True, text=True, timeout=_TIMEOUT)
    except subprocess.TimeoutExpired:
        log.warning("sandbox scan timed out", extra={"context": {"repo": full_name}})
        return None
    except (OSError, subprocess.SubprocessError) as exc:
        log.warning("sandbox scan failed to run",

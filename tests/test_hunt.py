...
def test_hunt_progress_reporter_fires_each_stage(tmp_path):
    # The human progress channel is display-only, but its stage callbacks must
    # fire around a real confirmation so the CLI renderer has something to show.
    # A recording double locks that contract without needing a TTY.
    from git_warden.enums import ArtifactType, FeedSource
    from git_warden.models import MaliciousArtifact
    from git_warden.progress import NullProgress

    class Rec(NullProgress):
        def __init__(self):
            self.calls = []

        def phase(self, title, detail=""):
            self.calls.append(("phase", title))

        def discovery(self, by_method, total):
            self.calls.append(("discovery", total))

        def screen_start(self, total):
            self.calls.append(("screen_start", total))

        def screen_end(self, screened):
            self.calls.append(("screen_end", screened))

        def tier2_start(self, total):
            self.calls.append(("tier2_start", total))

        def tier2_end(self, confirmed):
            self.calls.append(("tier2_end", confirmed))

        def confirmed(self, name, score):
            self.calls.append(("confirmed", name))

    db = Database.open(tmp_path / "prog.sqlite")
    db.start_run("seed", utcnow())
    db.upsert_artifact(MaliciousArtifact(
        artifact_type=ArtifactType.REPO, name="lurer/wallet-task", ecosystem="github",
        source=FeedSource.OPEN_SOURCE_MALWARE,
    ...
    rec = Rec()
    hunt(db, FakeClient(), TOOLS, run_id="prog", now=utcnow(), do_news=False,
         do_ioc=False, do_lineage=False, do_actor=False, do_enrich=False, do_osm=True,
         do_tier2=True, clone=clone_mal, progress=rec)

    kinds = [c[0] for c in rec.calls]
    assert ("phase", "Discover") in rec.calls
    assert "discovery" in kinds
    assert "screen_start" in kinds and "screen_end" in kinds
    assert "tier2_start" in kinds and "tier2_end" in kinds
    assert ("confirmed", "lurer/wallet-task") in rec.calls
    # A confirmation is display-only here, but it also grows the persisted corpus
    # the snapshot reports (the "it's iterative" signal the footer renders).
    assert db.corpus_snapshot()["confirmations"] >= 1
    db.close()


def test_console_progress_renders_without_a_tty():
    # ConsoleProgress against a StringIO (isatty=False) must not crash and must
    # surface the newcomer buckets + the learning delta the footer promises.
    import io

    from git_warden.progress import ConsoleProgress

    buf = io.StringIO()
    p = ConsoleProgress(buf)
    p.run_header(1, {"learned_iocs": 3, "code_signatures": 1,
                     "search_terms": 2, "confirmations": 5})
    p.phase("Discover", "x")
    p.source("IOC code search", 2, 2)
    p.discovery({"signature_match": 2}, 2)
    p.screen_start(2)
    p.screen_item(1, 2, "a/b", True)
    p.screen_item(2, 2, "c/d", False)
    p.screen_end(1)
    p.tier2_start(1)
    p.tier2_item(1, 1, "a/b")
    p.confirmed("a/b", 12)
    p.tier2_end(1)
    p.run_footer(
        {"learned_iocs": 3, "code_signatures": 1, "search_terms": 2},
        {"learned_iocs": 7, "code_signatures": 2, "search_terms": 5},
        {"candidates": 2, "candidates_by_method": {"signature_match": 2},
         "confirmed": 1, "gold_delivered": 1},
    )
    out = buf.getvalue()
    assert "run #1" in out
    assert "signatures matched" in out
    assert "[CONFIRMED] a/b" in out
    assert "Corpus grew" in out
